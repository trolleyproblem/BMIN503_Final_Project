...
compare_SPINC_performance <- function(regression_data,title) {
  ...
  print(paste0("Brier base (my fn): ",
               brier_score(as.numeric(glm_cv_preds$outcome)-1, 
                           glm_cv_preds$.pred_0)))
  ...
  print(paste0("Brier augmented (my fn): ",
               brier_score(as.numeric(glm_cv_preds_mu$outcome)-1, 
                           glm_cv_preds_mu$.pred_0)))
  ...
  overall_base_sbs <- scaled_brier_score(boot_data$outcome,
                                         boot_data$.pred_0_base)
  
  overall_mu_sbs <- scaled_brier_score(boot_data$outcome,
                                       boot_data$.pred_0_mu)
  
  print(paste0("Base SBS: ", overall_base_sbs, "; Mu SBS: ", overall_mu_sbs))
  
  # Function to calculate the scaled Brier Score
  # calculate_scaled_brier_score <- function(predictions, truth) {
  #   
  #   predictions <- boot_data$.pred_0_base
  #   truth <- boot_data$outcome
  #   
  #   # Calculate the Brier Score for the model
  #   brier_score <- brier_score(truth, predictions)
  #   
  #   # Calculate the Brier Score for a null model that uses the prevalence as the probability
  #   prevalence <- mean(truth)
  #   null_brier_score <- mean((prevalence - truth)^2)
  #   
  #   # Calculate the scaled Brier Score
  #   scaled_brier_score <- 1 - (brier_score / null_brier_score)
  #   return(scaled_brier_score)
  # }
  ...
  brier_diff <- function(data, indices) {
    ...
    brier_base <- scaled_brier_score(sampled_data$outcome, sampled_data$.pred_0_base)
    brier_mu <- scaled_brier_score(sampled_data$outcome, sampled_data$.pred_0_mu)
